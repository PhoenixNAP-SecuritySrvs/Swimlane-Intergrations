...
lrApiPassword = sw_context.inputs['lrApiPassword']
lrApiUrl = sw_context.inputs['lrApiUrl']
...
def swimlaneToLRStatusMapping(swimlaneStatus, swimlaneClosureReason):
    if swimlaneStatus == 'New Case':
        ...
        elif swimlaneClosureReason is None:
            return "True"
...
def buildLrComment(seRecord, seAppId, seRecordId):
    ...
    if len(seRecord['AP: Assigned To User(s)']) + len(seRecord['AP: Support User(s)']) > 1:
        ...
        for user in seRecord['AP: Assigned To User(s)']:
            users = user.name
    return 'Security Event Assigned to {} at {}. {}/record/{}/{}'.format(
        users, seRecord['TL: Acknowledged'].to_datetime_string(), sw_context.inputs['swimlaneWebUrl'], seAppId, seRecordId)
